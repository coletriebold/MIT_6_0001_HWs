...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        direction = "{}->{}".format(self.src, self.dest)
        lengths = " ({}, {})".format(self.total_distance, self.outdoor_distance)
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        if edge.get_source() not in self.nodes:
            raise ValueError("i did")
        elif edge.get_destination() not in self.nodes:
            raise ValueError("i did")
        else:
            edge_list = self.edges[edge.get_source()]
            #print(edge.str())
            edge_list.append(edge)
            self.edges[edge.get_source()] = edge_list
